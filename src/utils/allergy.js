...
const allergy = (givenName, familyName, callback) => {
  ...
  var query = "SELECT " +
                ...
                `WHERE json_extract_scalar(p.json,'$.name[0].family[0]')  = '${familyName}' ` +
                `AND json_extract_scalar(p.json,'$.name[0].given[0]') = '${givenName}'`
  ...
  client.execute({
      ...
      success: function(error, stats){
        console.log("success!")
          callback(undefined, queriedData)
      },
